...
# Add a function to test camera access
def realtime_hand_snake():
    model = YOLO('runs/detect_hand/vid001_to_vid012/weights/best.pt')
    ...
    while True:
        ...
        if len(bbox_array) > 0:
            ...
            cv2.circle(frame, (int(center[0]), int(center[1])), 5, (0, 255, 255), -1)  # Yellow color (0, 255, 255)
...
def test_camera():
    cap = cv2.VideoCapture(0)
    if not cap.isOpened():
        print("Failed to open camera. Trying a different camera index...")
...
def snake_hand(WIDTH, HEIGHT):
    ...
    def hand_tracking_thread():
        for hand_pos in realtime_hand_snake():
    ...
    while running:
        ...
        pygame.draw.rect(screen, (0, 0, 0), pygame.Rect(outline_x, outline_y, outline_width, outline_height), 2)
